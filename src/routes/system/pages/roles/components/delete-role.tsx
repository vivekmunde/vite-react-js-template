import type {
  TDeleteRoleRequestData,
  TDeleteRoleResponseData,
  TDeleteRoleResponseErrorCode,
} from '@/api-interfaces/system/roles/[roleId]/interfaces';
import type { TGetRole } from '@/api-interfaces/system/roles/interfaces';
import { useMutate } from '@/api/use-mutate';
import { SystemRBAC } from '@/components/rbac/system-rbac';
import { Translation } from '@/components/translation';
import { Alert, AlertTitle } from '@/components/ui/alert';
import {
  AlertDialog,
  AlertDialogBody,
  AlertDialogContent,
  AlertDialogFooter,
  AlertDialogHeader,
  AlertDialogTitle,
  AlertDialogTrigger,
} from '@/components/ui/alert-dialog';
import { Button } from '@/components/ui/button';
import { ButtonGroup } from '@/components/ui/button-group';
import { DataTestId } from '@/components/ui/data-test-id';
import { Flex } from '@/components/ui/flex';
import { Icon } from '@/components/ui/icon';
import { TypographyParagraph } from '@/components/ui/typography';
import { useApiErrorHandler } from '@/hooks/use-api-error-handler';
import { useSearchParams } from '@/hooks/use-search-params';
import { AlertCircleIcon, TrashIcon } from 'lucide-react';
import React from 'react';
import { useRolesQuery } from '../hooks/use-roles-query';

const DeleteRoleForm: React.FC<{
  role: TGetRole;
  onClose: () => void;
}> = ({ role, onClose }) => {
  const [{ loading: mutating, status, error }, mutator] = useMutate<
    TDeleteRoleRequestData,
    TDeleteRoleResponseData,
    string,
    TDeleteRoleResponseErrorCode
  >();
  useApiErrorHandler(status);
  const { onRevalidate: onRevalidateUserRoles } = useRolesQuery();

  const onDelete = () => {
    mutator.delete(`/system/roles/${role.id}`, undefined).then(({ status }) => {
      if (status === 'success') {
        onRevalidateUserRoles();
        onClose();
      }
    });
  };

  return (
    <React.Fragment>
      <AlertDialogHeader>
        <AlertDialogTitle>
          <Translation
            value="delete/deleteConfirmMessage"
            options={{ name: role.name }}
          />
        </AlertDialogTitle>
      </AlertDialogHeader>
      <AlertDialogBody>
        <TypographyParagraph>
          <Translation
            value="delete/deleteConfirmDescription"
            options={{ name: role.name }}
          />
        </TypographyParagraph>
      </AlertDialogBody>
      <AlertDialogFooter>
        <Flex direction={{ default: 'vertical' }} gap="normal">
          <ButtonGroup>
            <DataTestId value="delete">
              <Button
                variant="destructive"
                onClick={onDelete}
                loading={mutating}
              >
                <Translation value="common/delete" />
              </Button>
            </DataTestId>
            <DataTestId value="cancel">
              <Button variant="outline" onClick={onClose}>
                <Translation value="common/cancel" />
              </Button>
            </DataTestId>
          </ButtonGroup>
          {error?.message && (
            <Alert variant="destructive">
              <Icon>
                <AlertCircleIcon />
              </Icon>
              <AlertTitle className="--form-errors-alert-title">
                {error?.message}
              </AlertTitle>
            </Alert>
          )}
        </Flex>
      </AlertDialogFooter>
    </React.Fragment>
  );
};

const DeleteRole: React.FC<{
  role: TGetRole;
}> = ({ role }) => {
  const [searchParams, setSearchParams] = useSearchParams<{
    deleteRole?: string;
  }>({}, 'roles');

  const onOpen = () => {
    setSearchParams({ deleteRole: role.id });
  };

  const onClose = () => {
    setSearchParams({ deleteRole: undefined });
  };

  return (
    <SystemRBAC permissions={['MANAGE_ROLE']}>
      <DataTestId value="delete">
        <AlertDialog open={searchParams.deleteRole === role.id}>
          <AlertDialogTrigger asChild>
            <Button variant="ghost" onClick={onOpen}>
              <TrashIcon />
            </Button>
          </AlertDialogTrigger>
          <AlertDialogContent>
            <DeleteRoleForm role={role} onClose={onClose} />
          </AlertDialogContent>
        </AlertDialog>
      </DataTestId>
    </SystemRBAC>
  );
};

export { DeleteRole };
