...
import { useMySystemPermissions } from '@/hooks/use-my-system-permissions';
import { sortPermissions } from '@/utils/sort-permissions';
import React from 'react';
import { useFormContext } from 'react-hook-form';

const PermissionsField: React.FC = () => {
  const { data: systemPermissions } = useMySystemPermissions();
  const permissions = sortPermissions(
    systemPermissions?.items ?? ['READ_ORGANIZATION', 'READ_ROLE', 'READ_USER']
  );
